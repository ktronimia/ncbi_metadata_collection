...
# IMPORTANT:
# The NCBI search has already selected records by:
#   1. Organism name
#   2. Minimum sequence length
#   3. NOT UNVERIFIED in the title
#
# Therefore, at this stage we ONLY reject records that are explicitly
# marked as UNVERIFIED. No additional filtering is applied.

for record in all_records:

    # ONLY FILTER: EXCLUDE UNVERIFIED RECORDS
    ...
    # Record passes: retain it regardless of source feature,
    # organism qualifier, completeness, or other metadata.
    verified_records.append(record)

print(
    f"\\nRecords passing final filter: "
    f"{len(verified_records)}"
)

print(
    f"Records rejected (UNVERIFIED only): "
    f"{len(rejected_records)}"
)

if not verified_records:
    raise SystemExit(
        "\\nNo records passed the UNVERIFIED filter."
    )
